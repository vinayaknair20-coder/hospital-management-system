...
class Implementation(abstractdao):
        ...
        try:
            ...
            sql = f"""
                SELECT a.appointment_id, a.patient_id, p.patient_name, a.doctor_id,
                    a.token_number, a.appointment_date, a.status
            ...
            for row in rows:
                appointments.append(Appointments(
                    ...
                    specilaisation_id=None,               # hide from doctor
    ...
    def view_consultation_of_doctor(self, doctor_id):
        ...
        try:
            ...
            if not cursor.fetchone():
                return []
            sql = f"SELECT c.* FROM {self._t('table_consultation')} c JOIN {self._t('table_appointments')} a ON c.appointment_id=a.appointment_id WHERE c.doctor_id=%s"
            cursor.execute(sql, (doctor_id,))
            rows = cursor.fetchall()
            res = []
            for row in rows:
    ...
    def view_consultation(self, patient_id, doctor_id):
        ...
        try:
            cursor = self.conn.cursor(DictCursor)
            sql = f"SELECT c.* FROM {self._t('table_consultation')} c JOIN {self._t('table_appointments')} a ON c.appointment_id=a.appointment_id WHERE a.patient_id=%s AND a.doctor_id=%s"
    ...
    def get_or_create_prescription_for_appointment(self, appointment_id: int, consultation_id: int) -> int | None:
        """
        Checks if a prescription exists for a given appointment+consultation.
        ...
        try:
            ...
            # check existing
            cursor.execute(
                "SELECT prescription_id FROM prescriptions WHERE appointment_id=%s AND consultation_id=%s",
            ...
            # insert new
            cursor.execute(
                "INSERT INTO prescriptions (appointment_id, consultation_id) VALUES (%s, %s)",
                (appointment_id, consultation_id)
            )
            self.conn.commit()
    ...
    def create_med_prescription(self, med_pres: Med_prescriptions) -> bool:
        ...
        try:
            ...
            print("❌ Error inserting medicine prescription:", e)
            return False
        finally:
            if cursor:
                cursor.close()
    ...
    def create_test_prescription(self, test_pres: Test_prescriptions) -> bool:
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "INSERT INTO test_prescription (prescription_id, test_name, instructions) "
                "VALUES (%s, %s, %s)",
                (test_pres.prescription_id, test_pres.test_name, test_pres.instructions)
            )
            self.conn.commit()
            return cursor.rowcount == 1
        except Exception as e:
            print("❌ Error inserting test prescription:", e)
            return False
        finally:
            if cursor:
                cursor.close()
    ...
    def get_prescriptions_by_patient_id(self, patient_id: int) -> list:
        ...
        try:
            cursor = self.conn.cursor(DictCursor)
            sql = """
                SELECT prescription_id, appointment_id, consultation_id
                FROM prescriptions
    ...
    def get_test_prescriptions_by_prescription_id(self, prescription_id: int) -> list:
        ...
        try:
            cursor = self.conn.cursor(DictCursor)
            sql = "SELECT test_name, instructions FROM test_prescription WHERE prescription_id=%s"
            cursor.execute(sql, (prescription_id,))
            return cursor.fetchall() or []
        except Exception as e:
